...
<html>
    ...
    <?php
        ...
        if (isset($_POST["btn_checkout"])) {
                ...
                $selC = "select * from tbl_booking where app_id='" .$_SESSION["app_id"]. "'and booking_status='0'";
                $rs = mysql_query($selC);
                $row=mysql_fetch_array($rs);
                
                $upQry1 = "update tbl_booking set booking_date=curdate(),booking_amount='".$amt."',booking_status='1' where app_id='" .$_SESSION["app_id"]. "'";
				mysql_query($upQry1);
...
<form method="post">
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            $sel = "select * from tbl_booking b inner join tbl_cart c on c.booking_id=b.booking_id where b.app_id='" .$_SESSION["app_id"]. "' and booking_status='0' and cart_status=0";
            $res = mysql_query($sel);
                while ($row=mysql_fetch_array($res)) {
                    ...
                    if ($rowpr=mysql_fetch_array($respr)) {
						...
						$chk=mysql_fetch_array(mysql_query($selstock1));
                        $resst = mysql_query($selstock);
                    if ($rowst=mysql_fetch_array($resst)) {
            ?>

            <div class="product">
                <div class="product-image">
                    <img
                        src="../Assets/File/ProductDocs/<?php echo $rowpr
                        ["product_image"]?>"
                        />
                </div>
